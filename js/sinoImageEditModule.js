...
1. alert 本工具采用浏览器最新特性实现，使用新版Chrome、Edge、Safari浏览器给您带来最完整的使用体验！
2. 批量操作 - imagePool
3. 支持多种格式
4. 通过WebAssembly在浏览器中在客户端处理它们。这意味着您的图片可以安全地保存在您自己的计算机上，而不是发送到某个地方的服务器，并且Squoosh即使在离线时也能正常工作。
...
window.sinoImageEditModule = (function () {
    var watermarkTextLeft= 0,watermarkTextTop= 0, watermarkTextWidth=100;//水印文字左边缘，上边缘，宽和高
    ...
    var init=function(param) {
        ...
        uploadIcon=document.getElementById('el-upload-dragger');

        const handleDrag = (e,bool)=> {
          e.preventDefault(); // 阻止默认事件
          loadtimes=0;
          
          var file = bool ? e.target.files[0] : e.dataTransfer.files[0]; //获取文件
        ...
        uploadIcon.addEventListener('drop', (e)=> handleDrag(e), false);
        uploadIcon.addEventListener('change', (e)=> handleDrag(e,true), false);
